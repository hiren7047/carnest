...
const CarCard = ({ car, large = false }: { car: CarView; large?: boolean }) => {
  const carDetailPath = useAppPath(`/cars/${car.id}`);
  const bookPath = useAppPath(`/cars/${car.id}?book=1`);

  return (
    <Link to={carDetailPath} className="group block min-w-0 w-full max-w-full">
      <div className="flex h-full min-w-0 flex-col overflow-hidden rounded-xl border border-border/50 bg-card shadow-sm transition-shadow hover:shadow-md">
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-between gap-3 p-3.5 sm:p-4">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:gap-2">
            ...
            >
              <Link to={carDetailPath}>View Details</Link>
            </Button>
            <Button
              variant="outline"
              size="sm"
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <Link to={bookPath}>Book Test Drive</Link>
            </Button>
          </div>
        </div>
      </div>
